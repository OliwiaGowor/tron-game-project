...
class AggressiveAgent(Agent):
    def __init__(self, player_id, start_pos, symbol, watchtower):
        super().__init__(player_id, start_pos, symbol, watchtower)

    def decide_move(self):
        # Aggressive agent decyzje są oparte na agresywnej logice z wieży
        self.direction = self.watchtower.get_best_aggressive_move(self)
...
class Watchtower:
    ...
    def get_best_aggressive_move(self, agent):
        x, y = agent.position
        directions = {
            "UP": (0, -1),
            "DOWN": (0, 1),
    ...
    def evaluate_aggressive_move(self, x, y, agent):
        """
        ...
        elif not (0 <= x < self.game.width and 0 <= y < self.game.height):
...
def create_game(mode):
    ...
    if mode == "user_vs_agents":
        ...
        agent1 = AggressiveAgent(2, game.random_empty_position(), 'A1', watchtower)
        agent2 = Agent(3, game.random_empty_position(), 'A2', watchtower)
        game.players = [player1, agent1, agent2]
    
    elif mode == "agents_only":
        agent1 = AggressiveAgent(1, game.random_empty_position(), 'A1', watchtower)
        agent2 = AggressiveAgent(2, game.random_empty_position(), 'A2', watchtower)
        agent3 = Agent(3, game.random_empty_position(), 'A3', watchtower)
        agent4 = Agent(4, game.random_empty_position(), 'A4', watchtower)
